...
describe "opscode-account user association", :association do
  ...
  context "/users/USER/organizations endpoint" do
    ...
    context "invoking as " do
      ...
      it "an admin org user succeeds (because the admin has read access to the user)" do
        pending "Security: admin member of org can currently see all THis is a security hole: an admin member of an org can see what orgs any member user is in "
        get(user_org_url, platform.admin_user).should look_like({:status => 403})
      end
      it "another org member fails" do
        get(user_org_url, platform.non_admin_user).should look_like({:status => 403})
      end
      it "a neutral third party fails" do
        get(user_org_url, platform.bad_user).should look_like({:status => 403})
      end
    end
  end
